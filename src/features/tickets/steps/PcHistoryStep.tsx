...
export function PcHistoryStep({ ticket }: PcHistoryStepProps) {
  const { data, isLoading } = useAsync<{ tickets: Ticket[] }>(() => gql(TICKETS_QUERY, {}), []);
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                {["ID", "Título", "Tipo", "Estado", "Asignado a", "Diagnóstico", "Fecha"].map(
                  (h) => (
                    <th
                      key={h}
            ...
            <tbody>
              {history.map((t) => {
                ...
                return (
                  <tr
                    key={t.id}
                    className="border-b border-border last:border-0 hover:bg-black/2 transition-colors"
                  >
