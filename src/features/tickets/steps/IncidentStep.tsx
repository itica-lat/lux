...
export function IncidentStep({ ticket }: IncidentStepProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-base font-semibold text-[#1d1d1f] dark:text-white mb-4">Información del incidente</h2>
      ...
      <div>
        <span className="text-xs text-[#86868b] uppercase tracking-widest font-medium block mb-1.5">Título</span>
        <p className="text-sm font-medium text-[#1d1d1f] dark:text-white">{ticket.title}</p>
      </div>
      <div>
        <span className="text-xs text-[#86868b] uppercase tracking-widest font-medium block mb-1.5">Descripción</span>
        <p className="text-sm text-[#1d1d1f] dark:text-white leading-relaxed">{ticket.description}</p>
      </div>
    </div>
  );
}
