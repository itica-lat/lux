...
export function PcStatusStep({ ticket }: PcStatusStepProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-base font-semibold text-[#1d1d1f] dark:text-white mb-1">Estado del equipo</h2>
        <p className="text-xs text-[#86868b]">{equipment.location} · {equipment.serialNumber}</p>
      </div>
      ...
      {equipment.issues && (
        <div>
          <span className="text-xs text-[#86868b] uppercase tracking-widest font-medium block mb-1.5">Fallas registradas</span>
          <p className="text-sm text-[rgb(255,159,10)]">{equipment.issues}</p>
        </div>
      )}

      <div>
        <h3 className="text-xs text-[#86868b] uppercase tracking-widest font-medium mb-3">
              ...
              <tbody>
                {equipment.components.map(c => (
                  <tr key={c.id} className="border-b border-black/5 dark:border-white/4 last:border-0 hover:bg-black/2 transition-colors">
                    <td className="px-4 py-2 text-sm text-[#1d1d1f] dark:text-white">{c.name}</td>
                    <td className="px-4 py-2 text-xs text-[#86868b]">{c.model}</td>
                    <td className="px-4 py-2 text-xs text-[#86868b]">{c.manufacturer}</td>
                    <td className="px-4 py-2 text-xs text-[#86868b] font-mono">{c.serialNumber}</td>
                    <td className="px-4 py-2 text-xs text-[#86868b] font-mono">{c.partNumber}</td>
