...
export function TicketWizard({ ticket, onComplete, canComplete }: TicketWizardProps) {
 ...
 <Button
 variant="secondary" size="sm" onClick={() => setCurrentStep(s => Math.max(0, s - 1))}
 ...
 <Button
 size="sm" onClick={() => setCurrentStep(s => Math.min(steps.length - 1, s + 1))}
 >
 Siguiente
 </Button>
 ) : canComplete && !isResolved ? (
 <Button
 size="sm" onClick={handleComplete}
