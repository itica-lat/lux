...
export function EquipmentStatus() {
 ...
 <p className="text-sm text-muted-foreground mt-0.5">Distribución y estado de equipos por ubicación</p>
 </div>
 ...
 <td className="px-6 py-4 text-xs text-muted-foreground">{new Date(p.updatedAt).toLocaleDateString('es-UY')}</td>
 <td className="px-6 py-4 text-sm text-muted-foreground">{p.kind}</td>
 <td className="px-6 py-4">
 {p.issues ? (
 <span className="text-xs text-warning truncate max-w-[160px] block">{p.issues}</span>
 ) : (
